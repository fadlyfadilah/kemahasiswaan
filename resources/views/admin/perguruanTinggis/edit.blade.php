...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.perguruanTinggi.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.perguruan-tinggis.update", [$perguruanTinggi->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nama">{{ trans('cruds.perguruanTinggi.fields.nama') }}</label>
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $perguruanTinggi->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('prodis') ? 'is-invalid' : '' }}" name="prodis[]" id="prodis" multiple>
                    @foreach($prodis as $id => $prodi)
                        <option value="{{ $id }}" {{ (in_array($id, old('prodis', [])) || $perguruanTinggi->prodis->contains($id)) ? 'selected' : '' }}>{{ $prodi }}</option>
                    @endforeach
                </select>
